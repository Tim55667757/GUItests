...
# This is file with one functional test-case.
...
def TestCase1(startURL, login, password, opTimeout=5000, browserString='*firefox', ffProfile=None, instance=0):
    """
    This is example of test-case 1.
    """
    try:
        startOperation = steps_lib.datetime.now()
        steps_lib.LOGGER.info('Thread #%s, TestCase1, start time: %s' %
        ...
        # Step 1. Verify elemens present.
        status += steps_lib.VerifyMainPageContainsSomeElements(opTimeout, instance)

        # ----- Return results and status
        finishOperation = steps_lib.datetime.now()
        steps_lib.LOGGER.info('Thread #%s, TestCase1, finish time: %s' % (str(instance), steps_lib.FormatTimeString(finishOperation)))
        steps_lib.LOGGER.info('Thread #%s, TestCase1, duration: %s' % (str(instance), str(finishOperation - startOperation)))
        ...
        if status == 0:
            steps_lib.LOGGER.info('Thread #%s, TestCase1, status: PASS' % str(instance))
        else:
            steps_lib.LOGGER.info('Thread #%s, TestCase1, status: FAIL' % str(instance))
        ...
        steps_lib.LOGGER.error('Thread #%s, TestCase1, status: error' % str(instance))
...
def Main():
    """
    Running only one thread with testcase1.
    """
    try:
        steps_lib.LOGGER.info('Thread #0, ===== Starting 1 thread of test-case TestCase1 =====')
